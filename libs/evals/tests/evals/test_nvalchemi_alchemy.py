...
def _run_task(task, model: BaseChatModel) -> dict:
    ...
    query = (
        ...
        "positions/energies/forces/losses/graph sizes): save the actual "
        ...
        "these tensors and re-verifies the physics claims numerically — "
        "a `result.json` alone cannot pass for tensor tasks. If the task "
        "tracks positions over time, save `positions_initial` and "
        "`positions_final`; if it tracks energy, save the full per-step "
        "`energies` trace; if it computes losses, save `losses`; if it "
        "builds graphs, save per-graph `sizes`.\n"
